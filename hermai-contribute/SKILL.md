...
```json
{
  "meta": {
    "skill_update": {
      "current": "1.0.0",
      "latest": "1.2.0",
      "install_cmd": "npx skills add hermai-ai/hermai-skills --skill hermai-contribute",
...
When you see this field, tell the user in one short sentence before carrying on with the task — e.g. *"Heads up: your hermai-contribute skill (1.0.0) is behind 1.2.0. Run `npx skills add hermai-ai/hermai-skills --skill hermai-contribute` to upgrade."* If `meta.skill_update` is absent, the user is current — no nudge needed.
