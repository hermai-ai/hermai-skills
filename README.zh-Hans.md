...
两类用户(调用者与贡献者)都由单一的 `hermai` skill 通过 progressive disclosure 服务 — 贡献者用的 references 放在 `references/contribute/` 下,仅在需要时才加载。
...
Skill 是一个目录,内含一份带有 YAML frontmatter(`name`、`version`、`description`)的 `SKILL.md`。`references/` 下的文件通过 progressive disclosure 按需加载 — 在 skill 明确引导之前,不会占用 agent 的 context。
